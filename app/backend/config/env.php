...
if (!$jwtSecret) {
    ...
    if (in_array($appEnv, ['production', 'prod', 'staging'], true)) {
        throw new RuntimeException('JWT_SECRET must be set in the environment for production/staging.');
    }

    $jwtSecret = bin2hex(random_bytes(32));
    $line = 'JWT_SECRET="' . $jwtSecret . '"' . PHP_EOL;
    $canWrite = (file_exists($envFile) && is_writable($envFile)) || (!file_exists($envFile) && is_writable(__DIR__));
    if (!$canWrite) {
        throw new RuntimeException('JWT_SECRET is missing and config/.env is not writable. Set JWT_SECRET in config/.env.');
    }
    if (file_exists($envFile)) {
